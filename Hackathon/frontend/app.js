...
(function () {
    ...
    vapiInstance.on('message', (msg) => {
      if (msg.type === 'transcript' && msg.transcriptType === 'final') {
        if (msg.role === 'user') {
          ...
          addMessage(msg.transcript, false);
        }
      }
    });
  ...
  async function sendToBackend(userText) {
    ...
    try {
      ...
      if (result.urgency) {
        responseHtml = '<div style="background:#fef2f2;border-left:4px solid #ef4444;padding:8px 12px;border-radius:6px;margin-bottom:8px;">' +
          '<strong style="color:#ef4444;">EMERGENCY</strong></div>' + responseHtml;
      }
